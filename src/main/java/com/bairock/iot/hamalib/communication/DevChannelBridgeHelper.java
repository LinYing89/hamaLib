package com.bairock.iot.hamalib.communication;
	...
	private DevChannelBridge getDevChannelBridge(Device dev, String userName, String groupName) {
		...
		for (DevChannelBridge db : listDevChannelBridge) {
			if(db.getDevice() != null){
				if(db.getDevice().getDevGroup().getUser().getUsername().equals(userName)
						&& db.getDevice().getDevGroup().getGroupName().equals(groupName)) {
					if(devInDevChannelBridge(db, dev)) {
						return db;
					}
				}
...
//		}
	...
	public void channelInActive(String channelId) {
		...
		for (DevChannelBridge db : list) {
			if(db != null && db.getChannelId() != null) {
                if(db.getChannelId().equals(channelId)) {
					setDeviceStatus(db.getDevice(), DevStateHelper.DS_YI_CHANG);
                    removeBridge(db);
                }
            }else {
            	removeBridge(db);
	...
	public void setDeviceStatus(Device dev, String devState) {
		if (null != dev) {
			dev.setStatus(devState);
		}
	}
